...
def stop_camera_worker(camera_id: int):
    ...
    if camera_id in _camera_threads:
        ...
        # Do NOT del _camera_threads[camera_id] here, we need it to join below
        # Del only if necessary in sync_camera_workers or after joining
...
def sync_camera_workers():
    ...
    try:
        ...
        # Clean up threads that have naturally died
        for cam_id, thread in list(_camera_threads.items()):
            if not thread.is_alive():
                print(f"Worker for camera {cam_id} died naturally. Removing from list.")
...
async def lifespan(app) -> AsyncGenerator[None, None]:
    ...
    # --- FIX for Ctrl+C Hang: Gracefully join workers (max 3 seconds) ---
    print("Waiting for camera workers to join (max 3s timeout per worker)...")
    SHUTDOWN_TIMEOUT_PER_WORKER = 3
    
    for cam_id, thread in list(_camera_threads.items()):
        if thread.is_alive():
            try:
                # Give the worker a chance to finish its I/O cleanup
                thread.join(timeout=SHUTDOWN_TIMEOUT_PER_WORKER)
                if thread.is_alive():
                    print(f"Warning: Worker thread {cam_id} timed out during join. Forcing exit.")
            except Exception:
                pass
            finally:
                if cam_id in _camera_threads:
                    del _camera_threads[cam_id] # Clean up the dictionary entry
...
# create FastAPI app with lifespan (Note: must be created once)
